 ...
 * @LastEditTime: 2024-05-01 21:51:28
...
static esp_err_t osh_fsm_create_state(OSH_FSM_STATES_ENUM state,
    ...
    // create
    osh_FSM_state_stru *tmp_state = malloc(sizeof(osh_FSM_state_stru));
    if (NULL == tmp_state) {
    ...
    // init
    memset(tmp_state, 0, sizeof(osh_FSM_state_stru));
...
esp_err_t osh_fsm_fetch_state(OSH_FSM_STATES_ENUM state,
    ...
    osh_FSM_state_stru *tmp_state = NULL;
    listFOR_EACH_ENTRY(&g_osh_FSM->states_list, osh_FSM_state_stru, tmp_state) {
        if (state == tmp_state->state) {
            ESP_LOGD(FSM_TAG, "find state %d", (int)state);
            *fsm_state = tmp_state;
            break;
        }
    }
...
esp_err_t osh_fsm_get_event(osh_FSM_state_stru *fsm_state,
    ...
    osh_FSM_event_stru *tmp_event = NULL;
    listFOR_EACH_ENTRY(&fsm_state->events_list, osh_FSM_event_stru, tmp_event) {
    ...
    if (NULL == tmp_event) {
        ESP_LOGE(FSM_TAG, "can't find event %d at state %d",
...
esp_err_t osh_fsm_create_event(osh_FSM_state_stru *fsm_state, EventBits_t bits,
    ...
    // create
    osh_FSM_event_stru *tmp_event = malloc(sizeof(osh_FSM_event_stru));
    if (NULL == tmp_event) {
    ...
    // init
    memset(tmp_event, 0, sizeof(osh_FSM_event_stru));
...
esp_err_t osh_init_FSM(void *arg)
{
    ...
    if (NULL != g_osh_FSM->init_callback) {
        ...
        return g_osh_FSM->init_callback(arg);
    }
    return ESP_OK;
}
...
esp_err_t osh_FSM_loop_step(void *arg)
{
    ...
    if (ESP_OK != (res = osh_fsm_fetch_state(g_osh_FSM->current_state, &fsm_state))
        ...
        if (NULL != g_osh_FSM->miss_callback) {
            ...
            return g_osh_FSM->miss_callback(arg);
        }
    ...
    osh_FSM_event_stru *fsm_event = NULL;
    listFOR_EACH_ENTRY(&fsm_state->events_list, osh_FSM_event_stru, fsm_event) {
        if (0 != (bits & fsm_event->bits)) {
            ...
            ESP_LOGI(FSM_TAG, "state %d event %d callback...",
                    (int)fsm_state->state, (int)fsm_event->bits);
            if (ESP_OK != (res = fsm_event->callback(fsm_event->arg, arg))) return res;
...
esp_err_t osh_fini_FSM(void *arg)
{
    ...
    if (NULL != g_osh_FSM->fini_callback) {
        ...
        if (ESP_OK != (res = g_osh_FSM->fini_callback(arg))) return res;
    }
    ...
    if (!listLIST_IS_EMPTY(&g_osh_FSM->states_list)) {
        // not empty list
        osh_FSM_state_stru *fsm_state = NULL;
        listFOR_EACH_ENTRY(&g_osh_FSM->states_list, osh_FSM_state_stru, fsm_state) {
            if (!listLIST_IS_EMPTY(&fsm_state->events_list)) {
                // not empty list
                osh_FSM_event_stru *fsm_event = NULL;
                listFOR_EACH_ENTRY(&fsm_state->events_list, osh_FSM_event_stru, fsm_event) {
